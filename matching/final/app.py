...
class KNRM(torch.nn.Module):
    def __init__(
        ...
        self.embeddings = torch.nn.Embedding.from_pretrained(
            torch.load(emb_path)["weight"], freeze=True, padding_idx=0
        )
...
class Helper:
    ...
    def prepare_model(self):
        self.model = KNRM(emb_path=self.emb_path_knrm, mlp_path=self.mlp_path)
    ...
    def get_suggestion(
        self, query: str, ret_k: int = 10, ann_k: int = 100
    ) -> List[Tuple[str, str]]:
