...
class KNRM(torch.nn.Module):
    ...
    def _get_matching_matrix(
        self, query: torch.Tensor, doc: torch.Tensor
    ) -> torch.FloatTensor:
        # shape = [B, L, D]
...
class Solution:
    ...
    def get_all_tokens(
        self, list_of_df: List[pd.DataFrame], min_occurancies: int
    ) -> List[str]:
        def flatten(t):
    ...
    def create_glove_emb_from_file(
        ...
        for idx, word in enumerate(inner_keys):
            ...
            if word in glove_emb:
                ...
                matrix[idx] = np.random.uniform(
                    -rand_uni_bound, rand_uni_bound, size=out_dim
                )
    ...
    def create_val_pairs(
        ...
        groups = inp_df_select[
            inp_df_select.id_left.isin(glue_dev_leftids_to_use)
        ].groupby("id_left")
    ...
    def ndcg_k(
        self, ys_true: np.array, ys_pred: np.array, ndcg_top_k: int = 10
    ) -> float:
    ...
    def valid(
        self, model: torch.nn.Module, val_dataloader: torch.utils.data.DataLoader
    ) -> float:
        ...
        for cur_id in labels_and_groups.left_id.unique():
            ...
            ndcg = self.ndcg_k(
                cur_df.rel.values.reshape(-1), cur_df.preds.values.reshape(-1)
            )
            if np.isnan(ndcg):
                ndcgs.append(0)
            else:
