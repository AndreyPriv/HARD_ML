...
class Solution:
    def __init__(
        ...
        self.features_to_choice = int(self.num_input_features * colsample_bytree)
    ...
    def _scale_features_in_query_groups(
        self, inp_feat_array: np.array, inp_query_ids: np.array
    ) -> np.array:
    ...
    def _train_one_tree(
        self, cur_tree_idx: int, train_preds: torch.FloatTensor
        ...
        tree.fit(
            self.X_train[this_tree_objs.reshape(-1)][:, this_tree_feats].numpy(),
    ...
    def _calc_data_ndcg(
        ...
        for cur_id in np.unique(queries_list):
            ...
            cur_ndcg = self._ndcg_k(true_labels[mask], preds[mask], self.ndcg_top_k)
    ...
    def fit(self):
        ...
        for cur_tree_idx in p_bar:
            tree, this_tree_feats = self._train_one_tree(cur_tree_idx, train_preds)
            self.trees.append(tree)
            self.trees_feat_idxs.append(this_tree_feats)

            cur_tree_train_data = self.X_train[:, this_tree_feats].numpy()
            train_preds += self.lr * torch.FloatTensor(
                tree.predict(cur_tree_train_data)
            ).reshape(-1, 1)
            ...
            test_preds += self.lr * torch.FloatTensor(
                tree.predict(cur_tree_test_data)
            ).reshape(-1, 1)
