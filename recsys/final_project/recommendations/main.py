...
RECOMMENDATION_SOURCE = Counter(
    "recommendation_source", "Source of recommendations", ["source"]
)
...
def add_unseen_random_items(
    item_ids: List[str], unseen_random_items: List[str], source: str
):
...
async def get_recs(user_id: str):
    with REQUEST_LATENCY.labels(endpoint="/recs").time():
        ...
        try:
            ...
            lightfm_items = await redis_connection.json().get(
                f"lightfm_recommendations:{user_id}"
            )
            unseen_random_items = await redis_connection.json().get(
                f"unseen_random_items:{user_id}"
            )
        except redis.exceptions.ConnectionError:
        ...
        if lightfm_items and ENABLE_LIGHTFM_RECS:
            ...
            item_ids, source = add_unseen_random_items(
                item_ids, unseen_random_items, source
            )
        ...
        if random.random() < EPSILON:
            item_ids = np.random.choice(
                list(unique_item_ids), size=TOP_K, replace=False
            ).tolist()
            ...
            item_ids.extend(
                np.random.choice(
                    list(unique_item_ids - set(item_ids)),
                    size=TOP_K - starting_size,
                    replace=False,
                ).tolist()
            )
        ...
        logging.info(
            f"Final recommendations for user {user_id}:{item_ids} Source: {source}"
        )
